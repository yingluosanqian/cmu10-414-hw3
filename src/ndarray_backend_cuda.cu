...
__device__ size_t getPos(CudaVec shape, CudaVec strides, size_t gid) {
  size_t pos = 0;
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  if (gid < size) {
    out[gid] = a[offset + getPos(shape, strides, gid)];
  }
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    out[offset + getPos(shape, strides, gid)] = a[gid];
  }
}
...
void EwiseSetitem(const CudaArray& a, CudaArray* out, std::vector<int32_t> shape,
   ...
   */
  /// BEGIN SOLUTION
  CudaDims dims = CudaOneDim(a.size);
  EwiseSetitemKernel<<<dims.grid, dims.block>>>(a.ptr, out->ptr, a.size, VecToCuda(shape),
                                                VecToCuda(strides), offset);
  /// END SOLUTION
}
...
__global__ void ScalarSetitemKernel(scalar_t val, scalar_t* out, size_t size, CudaVec shape,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    out[offset + getPos(shape, strides, gid)] = val;
  }
}
...
void ScalarSetitem(size_t size, scalar_t val, CudaArray* out, std::vector<int32_t> shape,
   ...
   */
  /// BEGIN SOLUTION
  CudaDims dims = CudaOneDim(size);
  ScalarSetitemKernel<<<dims.grid, dims.block>>>(val, out->ptr, size, VecToCuda(shape),
      VecToCuda(strides), offset);
  /// END SOLUTION
}
...
#define MM_BLOCKDIM_X 16
#define MM_BLOCKDIM_Y 16
#define VX 2
#define VY 2
#define S 32
#define L 32  // MM_BLOCKDIM_Y * VY

__global__ void MatmulKernel(const scalar_t* __restrict__ A, const scalar_t* __restrict__ B,
                             scalar_t* __restrict__ C, size_t M, size_t N, size_t P) {
  __shared__ scalar_t sA[S][L], sB[S][L];
  scalar_t c[VY][VX] = {0}, a[VY], b[VX];
  size_t base_x = blockIdx.x * blockDim.x + threadIdx.x;
  size_t base_y = blockIdx.y * blockDim.y + threadIdx.y;
  for (size_t i = 0; i < N; i += S) {
    __syncthreads();
    size_t total_threads = blockDim.y * blockDim.x;
    size_t tid = threadIdx.y * blockDim.x + threadIdx.x;
    for (size_t j = 0; j < S * L / total_threads; j++) {
      size_t row = ((j * total_threads) + tid) / L;
      size_t col = ((j * total_threads) + tid) % L;
      size_t Ar = blockIdx.y * L + col, Ac = i + row;
      size_t Br = i + row, Bc = blockIdx.x * L + col;
      sA[row][col] = (Ar < M && Ac < N) ? A[Ar * N + Ac] : 0;
      sB[row][col] = (Br < N && Bc < P) ? B[Br * P + Bc] : 0;
    }
//    sA[:] = A[i: i + S, blockIdx.y * L, (blockIdx.y + 1) * L];
...
//  C[base_y * V: (base_y + 1) * V, base_x * V: (base_x + 1) * V] = c;
}
